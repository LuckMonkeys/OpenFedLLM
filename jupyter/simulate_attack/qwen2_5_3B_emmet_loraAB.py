...
ft_pure = "./attack/edit/hparams/EMMET/qwen2.5-3b_lora_ffn_AB.yaml"
...
layers_list = [[4,5,6,7,8], [12,13,14,15,16]]
...
for layers in layers_list:
    ...
    name = "_".join(map(str, layers))
    
    alg_name, asr, meteor, eval_metrics_after = test_attack(ft_pure, model=model, tok=tok, override_params=override_params) 
    result_dict[name] =  [alg_name, asr, meteor, eval_metrics_after]    
    
save_result(result_dict=result_dict, save_dir="simulate_attack/eval_results", save_name="qwen2_5_3B_emmet_loraAB")

breakpoint()


# CUDA_VISIBLE_DEVICES=7 python simulate_attack/qwen2_5_3B_emmet_loraAB.py
